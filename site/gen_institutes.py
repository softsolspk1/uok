...
REAL_URLS = {
    'cdfst': 'research_institutes/cdfst/index.php', 'asce': 'research_institutes/asce/index.php',
    'cmg': 'research_institutes/cmg/index.php', 'cews': 'research_institutes/cews/index.php',
    'chwb': 'research_institutes/chwb/index.php', 'confucius': 'research_institutes/confucius/index.php',
    'kibge': 'research_institutes/kibge/index.php', 'ies': 'research_institutes/ies/index.php',
    'ims': 'research_institutes/ims/index.php', 'isst': 'research_institutes/isst/index.php',
    'nnrc': 'research_institutes/nnrc/index.php', 'psc': 'research_institutes/psc/index.php',
    'smbbc': 'research_institutes/smbbc/index.php', 'sympdc': 'research_institutes/sympdc/index.php',
}

LEADER_PATTERN = re.compile(
    r'^(Director General|Director|Chairman|Chairperson|In-?charge|Head|Coordinator|Convener|Focal Person)\s*:?\s*[:\-]?\s*(.+)$',
    re.IGNORECASE)

def parse_institute(slug, raw_text):
    lines = raw_text.split('\n')
    real_name = lines[2].strip() if len(lines) > 2 else slug
    ...
    leader_title, leader_name = None, None
    m = LEADER_PATTERN.match(body_lines[0])
    if m:
        leader_title = m.group(1).strip()
        leader_name = re.sub(r'\s*\(Acting\)\s*$', '', m.group(2)).strip()
        body_lines = body_lines[1:]
    if not body_lines:
        return None

    # Join everything into flowing text, cleaning stray tag remnants
    ...
    # Split into paragraphs on sentence boundaries, grouping ~2-3 sentences each
    ...
    return {
        'slug': slug, 'real_name': real_name, 'leader_title': leader_title,
        'leader_name': leader_name, 'paragraphs': paragraphs,
    }
...
.page-head h1{{font-size:clamp(1.8rem,3.8vw,2.7rem);line-height:1.12;margin-bottom:1rem;max-width:800px;}}
...
.fact-strip .fact b{{font-family:var(--font-display);font-size:1.1rem;}}
...
<main id="main">
  <div class="page-head"><div class="container reveal">
    <div class="breadcrumb"><a href="index.html">Home</a><span class="sep">/</span><a href="research.html">Research Institutes</a><span class="sep">/</span><span class="current">{name}</span></div>
    <div class="eyebrow">Research Institute</div>
    <h1 style="margin-top:.8rem;">{name}</h1>
    {lead}
  </div></div>
  ...
  <section class="band-inverse"><div class="container cta-band">
    <div><h3>Explore the rest of UoK research.</h3><p>Twenty institutes and centres, from marine biology to space science.</p></div>
    <a href="research.html" class="btn btn-primary">All Research Institutes</a>
  </div></section>
</main>
...
<footer class="band-inverse" id="footer"><div class="container">
  <div class="footer-grid">
    ...
    <div class="footer-col"><h4>Research</h4><ul><li><a href="research.html">All Institutes</a></li><li><a href="journals.html">Journals</a></li></ul></div>
    <div class="footer-col"><h4>Academics</h4><ul><li><a href="faculties.html">All Faculties</a></li><li><a href="library.html">Library</a></li></ul></div>
    <div class="footer-col"><h4>Administration</h4><ul><li><a href="administration.html">Vice Chancellor</a></li><li><a href="administration.html">ORIC</a></li></ul></div>
...
def build_page(slug, data):
    name = data['real_name']
    body_html = '\n        '.join(f'<p>{esc(p)}</p>' for p in data['paragraphs'])
    lead = f'<p class="lead">{esc(data["paragraphs"][0][:200])}{"…" if len(data["paragraphs"][0]) > 200 else ""}</p>' if data['paragraphs'] else ''

    facts = []
    if data['leader_name']:
        facts.append(f'<div class="fact"><b>{esc(data["leader_name"])}</b><span>{esc(data["leader_title"] or "Lead")}</span></div>')
    ...
    <div class="container">
      ...
      <div class="contact-card reveal" style="margin-top:1.5rem;">
        <div class="crole">{esc(data["leader_title"] or "Lead")}</div>
        <div class="cname">{esc(data["leader_name"])}</div>
        <a href="research.html">Back to Research Institutes &rarr;</a>
      </div>
    </div>
  </section>'''

    return TEMPLATE.format(name=esc(name), lead=lead, fact_strip=fact_strip, body_html=body_html, contact_section=contact_section)
